...
namespace MiniRPG.Logic.Metagame
{
    ...
    public class MetagameSimulation : IMetagameSimulation
    {
        ...
        public BattleInitData StartBattle()
        {
            ...
            return new Logic.Battle.BattleInitData(
                new PlayerInitData(
                    0,
        ...
        public void OnBattleResult(BattleResult battleResult)
        {
            ...
            _profileController.Update(new IncreaseBattleCount(1));
            ...
            if(battleResult.winnerPlayerIndex == 0)
            {
                foreach(var unitResult in playerResult.unitResults)
                {
                    ...
                    {
                        int heroId = unitResult.heroId;
                        var hero = profile.GetHero(heroId);
                        if (hero == null)
                        {
                            _logger.LogError($"No hero with hero id : {heroId} found in player's profile.");
                            continue;
                        }
        ...
        private void IncreaseHeroXP(HeroData hero)
        {
            //increase hero xp
            _profileController.Update(new IncreaseHeroXP(hero.heroId, 1));
